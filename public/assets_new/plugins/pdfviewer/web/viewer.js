...
/***/ (function (module, exports, __webpack_require__) {
      ...
      function configure(PDFJS) {
        PDFJS.imageResourcesPath = base_url + 'assets/plugins/pdfviewer/web/images/';
        PDFJS.workerSrc = base_url + 'assets/plugins/pdfviewer/build/pdf.worker.js';
        PDFJS.cMapUrl = base_url + 'assets/plugins/pdfviewer/web/cmaps/';
      ...
      var PDFViewerApplication = {
        ...
        load: function load(pdfDocument) {
          ...
          $('#TotalPageCount').val(pdfDocument.numPages);
...
/***/ (function (module, exports, __webpack_require__) {
      ...
      function getParentForm(elem) {
        var parentli = findParentForm(elem);
        ...
        _createClass(PDFViewer, [{
          ...
          value: function update() {
            ...
            if (!this.isInPresentationMode) {
              ...
              //Custom
              $("#previousPage").val($('#diff').val())
              $('#diff').val(currentId);
              
              // Madhuri Code
                var OrderUID = $('#OrderUID').val();
                var current_document = $('#filter-questions option:selected').val();

                if (current_document && $("#previousPage").val() != $("#diff").val()) {
                  GetMappedDocumentsByPages(OrderUID,currentId);
                }
                // Madhuri
              var FindingPageNodes = $('ol').find('[data-category="' + currentId + '"]').length;
              if (FindingPageNodes == 1) {
                var subcat = $('ol').find('[data-category="' + currentId + '"]').parents('ol li').attr('data-documenttype');
                $("select.stackdropdown option[data-mastersubcategory='" + subcat + "']").prop("selected", true);

                // $('ol').find('[data-type="page"]').closest('li').css("background-color", "transparent");
                $('.highlight').find('input[type=checkbox]').prop('checked', false);
                $('.highlight').removeClass('highlight');
                var pageli = $('ol').find('[data-category="' + currentId + '"]').closest('li');
                $(pageli).addClass("highlight");
                $(pageli).find('input[type=checkbox]').prop('checked', true);
                // SubCategory li selector
                let subcategoryli = getParentForm($(pageli)[0]);
                if (subcategoryli) {

                  $('.categoryhighlight').removeClass('categoryhighlight')
                  $(subcategoryli).find('div:first').addClass('categoryhighlight');

                  if ($(subcategoryli).find('.nested').not('.active')) {
                    $(subcategoryli).find('.nested').addClass('active');
                  }
                  //console.log('Viewer Caret', $(subcategoryli).find('.right-caret'));
                  if ($(subcategoryli).find('.right-caret').not('.right-caret-down')) {
                    $(subcategoryli).find('.right-caret').addClass('right-caret-down');
                  }
                }

                // Category li selector
                let categoryli = getParentForm($(subcategoryli)[0]);

                if (categoryli) {}

                if ($(categoryli).find('.nested').not('.active')) {
                  $(categoryli).find('.nested').addClass('active');
                }
                //console.log('Viewer Caret', $(categoryli).find('.right-caret'));
                if ($(categoryli).find('.right-caret').not('.right-caret-down')) {
                  $(categoryli).find('.right-caret').addClass('right-caret-down');
                }

                // $('.checkbox_unsorted:checked').prop('checked', false);


              }
              else {
                //$("select.stackdropdown option[data-mastersubcategory='dummy']").attr("selected","selected");
              }
